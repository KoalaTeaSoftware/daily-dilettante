<template>
  <div id="storyWorlds">
    <b-container id="wessex-podcasts">
      ...
      <b-card v-for="book in books" class="series">
        <div class="bookDetails">
          <img class="mr-3"
               ...
               width="128"
          >
          <h4 class="mt-0">{{ book.title }}<span class="dueDate">({{ book.dueDate }})</span></h4>
          <p v-for="para in book.synopsis" class="book_synopsis">{{ para }}</p>
        </div>

        <div class="bookPodcasts">
          <div class="seasonPodcastList card-deck">
        ...
        .then(data => {
              ...
              itemList.forEach(item => {
                ...
                try {
                  ...
                  // This is creating new elements that do not exist at the start, so set() is essential
                  Vue.set(this.books[bookIndex].episodeList, episodeIndex, episodeDefinition)
                } catch (err) {
...
#storyWorlds {
  ...
  .bookDetails {
    overflow: auto; /* this forces the block containing the podcasts to cleat the image, even if there is not a lot of text*/
  }
